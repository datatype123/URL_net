...
model = pickle.load(open("/home/xuananle/Documents/URL_net/Backend/ML Models/model.pkl", "rb"))

with open("/home/xuananle/Documents/URL_net/Backend/data/test.txt") as fp:
    chrome_warning_contents = fp.read()
...
with open("/home/xuananle/Documents/URL_net/Backend/data/blacklist.txt") as fp:
    lists = fp.read().splitlines()
with open("/home/xuananle/Documents/URL_net/Backend/data/whitelist.txt") as fp:
    white = fp.read().splitlines()
import tldextract
def main(url):
    new_url = tldextract.extract(url).domain + "." + tldextract.extract(url).suffix
    print("The new url is: ", new_url)
    if(lists.count(new_url) or lists.count(url)
       or lists.count("https://" + new_url + "/*") or lists.count("http://" + new_url + "/*")):
        return -1
    if(white.count(new_url) or white.count(url)):
        return 1
